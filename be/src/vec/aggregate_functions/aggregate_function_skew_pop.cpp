...
using SkewPopDataPT = SkewPopData<T, SkewPopName>;

AggregateFunctionPtr create_aggregate_function_skew_pop(const std::string& name,
    ...
    return creator_with_type_list<TYPE_TINYINT, TYPE_SMALLINT, TYPE_INT, TYPE_BIGINT, TYPE_LARGEINT,
            ...
            AggregateFunctionSkewKurt, SkewPopDataPT>(argument_types, result_is_nullable, attr);
}

void register_aggregate_function_skewness_pop(AggregateFunctionSimpleFactory& factory) {
    factory.register_function_both("skew_pop", create_aggregate_function_skew_pop);
}
